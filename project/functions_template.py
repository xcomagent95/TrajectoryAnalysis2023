...
def slidingWindowRecursive(traj, epsilon, start_index, result_list):
    ...
    for end_index in range(start_index + 2, len(traj) + 1):
        d = utils.perpendicularDistance(
            traj[end_index - 2], traj[start_index], traj[end_index - 1])
        if d > epsilon:
            result_list.append(traj[end_index - 2])
            return slidingWindowRecursive(traj, epsilon, end_index - 1, result_list)
...
def dynamicTimeWarping(firstTrajectory: trajectory, secondTrajectory: trajectory) -> float:
    ...
    distance_matrix = [[utils.euclideanDistance(
        point0, point1) for point1 in secondTrajectory] for point0 in firstTrajectory]

    # Initialize the cost matrix
    cost_matrix = [[float('inf')] * len(secondTrajectory)
                   for _ in range(len(firstTrajectory))]
    ...
    for i in range(1, len(firstTrajectory)):
        for j in range(1, len(secondTrajectory)):
            cost_matrix[i][j] = distance_matrix[i][j] + \
                min(cost_matrix[i-1][j], cost_matrix[i]
                    [j-1], cost_matrix[i-1][j-1])
