...
class mbb:
    ...
    def distancePointToMbb(self, point: point.point) -> float:
        ...
        """
        dx = max(abs(point.X - self.lowerLeft.X) -
                 (self.upperRight.X - self.lowerLeft.X) / 2, 0)
        dy = max(abs(point.Y - self.lowerLeft.Y) -
                 (self.upperRight.Y - self.lowerLeft.Y) / 2, 0)
...
def calculateSmallestMBB(nodeList: list) -> mbb:
    ...
    smallestMBB_lowerLeft_X = min(
        [node.value.lowerLeft.X for node in nodeList])
    smallestMBB_lowerLeft_Y = min(
        [node.value.lowerLeft.Y for node in nodeList])
    smallestMBB_upperRight_X = max(
        [node.value.upperRight.X for node in nodeList])
    smallestMBB_upperRight_Y = max(
        [node.value.upperRight.Y for node in nodeList])

    smallestMBB_lowerLeft = point.point(
        x=smallestMBB_lowerLeft_X, y=smallestMBB_lowerLeft_Y, timestamp=0.0)
    smallestMBB_upperRight = point.point(
        x=smallestMBB_upperRight_X, y=smallestMBB_upperRight_Y, timestamp=0.0)
    smallestMBB = mbb(lowerLeft=smallestMBB_lowerLeft,
                      upperRight=smallestMBB_upperRight)
...
class rTree:
    ...
    def findNode(self, nodeToStartFrom: node, newNode: node) -> node:
        ...
        # The function calls itself recursively after identifying the closest node
            ...
            for child in foundNode.children:
                distancesFromPointToChildren.append(
                    (child, child.value.distancePointToMbb(newNodesPoint)))
            nearestChild = min(distancesFromPointToChildren,
                               key=lambda child: child[1])  # nearestChild: tuple
    ...
    def splitNode(self, givenNode: node) -> None:
        ...
        if len(givenNode.children) <= 5:
            ...
            # Will contain tuples (group1, group2, total_area)
            partitionsAndAreaSizes = []
            ...
            for partition in subpartitions:
                ...
                remaining_points = [
                    childNode for childNode in nodesChildren if childNode not in partition]
                mbb2 = calculateSmallestMBB(list(remaining_points))
                mbb2_size = mbb2.getArea()

                total_area = mbb1_size + mbb2_size
                partitionsAndAreaSizes.append(
                    (list(partition), remaining_points, total_area))

            minPartition = min(partitionsAndAreaSizes,
                               key=lambda partition: partition[2])
            '''
			# FOR VISUALIZATION 
			# REQUIREMENTS: In the import both plotly packages have to be imported to visualize the partitions!
			# This code block was exclusively for debugging purpose. In case it needs to be executed, please install Pandas as a dependency.
            ...
            siblingNode = node(value=calculateSmallestMBB(
                minPartition[1]), children=minPartition[1], root=False, leaf=False)
            ...
            if givenNode.root:
                ...
                newParentNode = node(value=calculateSmallestMBB([givenNode, siblingNode]), children=[
                                     givenNode, siblingNode], root=False, leaf=False)
    ...
    def insertPoint(self, point: point.point) -> None:
        ...
        if self.root.children is None:
            ...
            newRoot = node(value=calculateSmallestMBB(nodeList=[
                           rootThatBecomesChild, newNode]), children=[rootThatBecomesChild, newNode], root=True)
        ...
        # If the tree has already more than 1 level:
            ...
            # returned node can only leaf or non-leaf
            nodeToAddNewNode = self.findNode(self.root, newNode)
            nodeToAddNewNode.children.append(newNode)
            nodeToAddNewNode.value = calculateSmallestMBB(
                nodeToAddNewNode.children)
